...
// {
extern 
void voie_unique_wprint(struct_print_t *  st_print)			/* Numero de la colonne de depart/fin d'affichage */
{

  //printf("voiciii le print %d\n",st_print->nb_total_trains);
  ecran_t * const ecran = st_print->ecran;   /* Ecran sur lequel on affiche                */
  voie_unique_t * const voie_unique = st_print->voie_unique;  /* Section a voie unique a afficher           */
  const train_id_t nb_total_trains = st_print->nb_total_trains; /* Nb total de trains circulant sur la ligne  */
  const train_id_t nb_max_trains = st_print->nb_max_trains; /* Max trains sur la voie unique              */
  int * x_debut = st_print->x_debut;
  ...
  for(i=0 , x=2 ; i<voie_unique->cpt[ZONE_ID_OUEST_OE] ; i++ , x=x+TAILLE_TRAIN_NCURSES )
    {
